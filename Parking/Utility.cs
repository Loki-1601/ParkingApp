    ...
    public class Utility
    {
        ...
        public static void EnterNewReservations()
        {
            using (var db = CreateDbContext())
            {
                ...
                while (isInputValid)
                {
                    ...
                    Utility.CreatePermitAssignmentsForReservations();
                    db.SaveChanges();
                    Utility.parkingStats();
        ...
        public static void parkingStats()
        {
            using (var db = CreateDbContext())
            {
                ...
                var totalReservations = todayReservations.Count();
                var totalPermits = db.ParkingAssignments.Count();
                var availablePermits = MaxPermits - totalPermits;
                var totalSysReservations = db.ParkingReservations.Count();

                Console.WriteLine($"Total reservations for today: {totalReservations}/{totalSysReservations}");
                Console.WriteLine($"Total permits issued for today: {totalPermits}");
                Console.WriteLine($"Available permits: {availablePermits}");
